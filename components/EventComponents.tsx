...
export const EventCard = ({ 
  ...
  return (
    <div
      ...
      className={`
        flex-shrink-0 
        ${isFlagship ? 'w-[95%] sm:w-[520px]' : 'w-[90%] sm:w-[450px]'} 
        group relative 
        ${isFlagship ? 'bg-gradient-to-br from-[#d0006f]/10 via-white/5 to-[#d0006f]/5' : 'bg-white/5'} 
        ${isHovered ? 'bg-white/10' : ''} 
        border-2 
        ${isFlagship 
          ? `${isHovered ? 'border-[#d0006f]' : 'border-[#d0006f]/50'}` 
    ...
    >
      ...
      {isFlagship && (
        <div className="absolute top-4 left-4 z-10">
          <div className="px-4 py-2 rounded-full bg-gradient-to-r from-[#d0006f] to-pink-600 backdrop-blur-sm border border-white/20">
  ...
  return (
    ...
    >
      <div className="relative">
        <div 
          className={`${markerSize} rounded-full transition-all ${isHovered ? 'ring-4' : ''}`} 
          style={{ 
        ...
        <div className={`absolute ${positionClass} left-1/2 -translate-x-1/2 whitespace-nowrap`}>
          <div 
            className={`${labelSize} rounded-lg`} 
            style={{ 
